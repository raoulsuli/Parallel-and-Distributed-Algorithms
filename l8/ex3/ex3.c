...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < MULTI; i++) v_recv[i] = 0;
    for (int i = 0; i < num_elements; i++) v_send[i] = 0;
    ...
    for (int i = 0; i < MULTI; i++) v_recv[i] += rank;
    ...
    if (rank == ROOT) {
        printf("Root process: ");
        for (int i = 0; i < num_elements; i++) printf("%d ", v_send[i]);
    }
    printf("\n");
